...
- **Daily / Weekly / Monthly** — same I/O · Parts · Tools. Parts/Tools use our hierarchy cats, bucketed by each turn’s time (not the whole session). Timeframe / grain apply to Time layout only. Session list is parent → sub-agent. **Hourglass** is a Gantt of duration (LLM/harness vs wait-for-user), with time-window zoom, pan, and drill-select. **← Back** returns from a session opened out of a period.
